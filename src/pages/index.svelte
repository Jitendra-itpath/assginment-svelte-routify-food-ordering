     ...
     {#each productData as product}
	     <div class="bg-white rounded-lg overflow-hidden shadow-lg border hover:border-gray-300">
			...
			<div class="px-6 py-4">
				...
				<button on:click={()=>{addToCart(product.productId,1)}} type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Add to cart</button>
			</div>
		</div>
     {/each}
</div>